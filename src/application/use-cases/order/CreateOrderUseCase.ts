...
export class CreateOrderUseCase {
  ...
  async execute(dto: CreateOrderDto): Promise<CreateOrderResponseDto> {
    ...
    // 3. Lấy danh sách sản phẩm và tồn kho tương ứng của các món được chọn
    const targetProductIds = itemsToOrder.map((item) => item.productId);
    const [inventories, dbProducts] = await Promise.all([
      this.inventoryRepositoty.findByProductIds(targetProductIds),
      this.productRepository.findByIds(targetProductIds),
    ]);
    ...
    // 9. Xóa các sản phẩm ĐÃ MUA ra khỏi giỏ hàng (giữ lại các sản phẩm chưa mua)
