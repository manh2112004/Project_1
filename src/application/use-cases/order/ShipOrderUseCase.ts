...
import { ShipOrderDto } from "../../dtos/order/ShipOrderDto";
import { CreateOrderResponseDto } from "../../dtos/order/CreateOrderDto";

export class ShipOrderUseCase {
  constructor(private readonly orderRepository: IOrderRepository) {}

  async execute(dto: ShipOrderDto): Promise<CreateOrderResponseDto> {
    const order = await this.orderRepository.findById(dto.orderId);
    if (!order) {
      throw new Error("Đơn hàng không tồn tại.");
    }

    order.ship(dto.shippingCode); // PROCESSING -> SHIPPED
